...
# --- الإعدادات الأساسية ---
ADMIN_PASSWORD = "5566"
# الرابط البرمجي للإرسال (تأكد من بقاء formResponse في النهاية)
...
def send_to_google(name, data_val, time_val, type_val, discount=0, overtime=0):
    # استخدام الـ IDs التي استخرجناها سابقاً
    payload = {
...
if user_role == "موظف":
    ...
    if entered_pass == STAFF_DATA[selected_name]["pass"]:
        ...
        # --- حساب الراتب اليومي (الميزة الجديدة) ---
        weekly_salary = STAFF_DATA[selected_name]['salary']
        daily_salary = int(weekly_salary / 7)  # الراتب الأسبوعي تقسيم 7 أيام
        
        st.subheader("💰 راتبك ومستحقاتك اليوم")
        col_s1, col_s2 = st.columns(2)
        col_s1.metric("الراتب الأسبوعي الصافي", f"{weekly_salary:,} د.ع")
        col_s2.metric("يوميتك (اليوم الصافي)", f"{daily_salary:,} د.ع")
        ...
        if col_b1.button("📥 تسجيل حضور"):
            emp = STAFF_DATA[selected_name]
            # تحديد وقت البداية للشفت (سواء شفت واحد أو أول شفت بالمزدوج)
            ...
            if discount > 0:
                st.error(f"تأخير {int(diff)} دقيقة. خصم اليوم: {discount:,} د.ع")
                st.info(f"الصافي الذي ستستلمه لليوم: {daily_salary - discount:,} د.ع")
            else:
                st.success(f"حضور في الوقت! راتبك اليوم: {daily_salary:,} د.ع")
            
            send_to_google(selected_name, c_date, c_time, "حضور", discount, 0)
            st.balloons()

        if col_b2.button("📤 تسجيل انصراف"):
            send_to_google(selected_name, c_date, c_time, "انصراف", 0, 0)
            st.info(f"تم تسجيل الانصراف الساعة {c_time}")
